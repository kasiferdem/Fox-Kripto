...
def node_deterministic_risk_policy(state: CryptoAgentState) -> Dict[str, Any]:
    ...
    for cand_item in candidates[:3]:
        ...
        vol_spike_min = float(strat_cfg.get("volume_spike_multiplier") or 1.15)
        tb_ratio_min = float(strat_cfg.get("min_taker_buy_pct") or 55.0)
        min_score_threshold = float(strat_cfg.get("min_ai_score") or 6.5)
        llm_approved = bool(eval_res and eval_res.get("is_scalp_recommended") and eval_res.get("potential_score", 0) >= 7.5)
        quant_approved = bool(vol_spike >= vol_spike_min and tb_ratio >= tb_ratio_min and (0.20 <= p_gain_5m <= 4.5) and v2_cand_score >= min_score_threshold)
    ...
    # 🛑 YENİ ALIM EMİRLERİ KİLİDİ (newBuyOrdersEnabled)
    if strat_cfg.get("new_buy_orders_enabled", True) is False:
        print(f"   🛑 [Kasa Koruma Kilidi]: new_buy_orders_enabled=False olduğu için {fresh_coin} alım emri açılmadı (Nakit Güvende).")
        return {"trade_proposal": None, "policy_check_passed": False, "human_approval": "Rejected", "coin_dna_analysis": coin_dna_analysis}
